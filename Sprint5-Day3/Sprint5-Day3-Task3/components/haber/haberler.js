...
Yazdığınız 'HaberYapici' component'ini kullanarak bir div.article öğesi oluşturun
 ve bunu DOM'daki div.articles düğümüne ekleyin. (bknz. index.html).


Adım 5: Yukarıdaki haberleri için data isimli diziye bulduğunuz bir teknoloji haberini eklemeyi deneyin.
 Diğer haber objeleri ile aynı yapıda olmasına dikkat edin.
*/
function HaberYapici(haberObj){
  ...
  h2.textContent = haberObj.baslik;
  articleDiv.appendChild(h2);

  const p = document.createElement("p");
  p.classList.add("tarih");
  p.textContent = haberObj.tarih;
  articleDiv.appendChild(p);

  const ilkp = document.createElement("p");
  ilkp.textContent = haberObj.ilkParagraf;
  articleDiv.appendChild(ilkp);

  const ikip = document.createElement("p");
  ikip.textContent = haberObj.ikinciParagraf;
  articleDiv.appendChild(ikip);

  const ucp = document.createElement("p");
  ucp.textContent = haberObj.ucuncuParagraf;
  articleDiv.appendChild(ucp);
