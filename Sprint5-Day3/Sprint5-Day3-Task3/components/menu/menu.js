...
Adım 3: Hala fonksiyon içindeyiz, DOM'dan menü butonunu seçin
 (`index.html` içinde `menu-button` sınıfına sahip element) ve buna  bir `click` event listener ekleyin. 
Butona her tıklanıldığında div.menu classına('menu' class'ına sahip olan div)
...
function MenuYapici(menuDizisi) {
  ...
  menuDizisi.forEach((element,index) => {
    const li = document.createElement("li");
    ul.appendChild(li);
    li.textContent = menuDizisi[index];
  });
